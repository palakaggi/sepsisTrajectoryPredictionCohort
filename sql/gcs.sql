...
--with all_charts as(
with gcs_eye as
(select hadm_id, subject_id, stay_id, charttime, valuenum as gcs_eye
        from chartevents
         where itemid=220739),
gcs_motor as
(select hadm_id, subject_id, stay_id, charttime, valuenum as gcs_motor
        from chartevents
        where itemid=223901),
gcs_verbal as
(
select hadm_id, subject_id, stay_id, charttime, valuenum as gcs_verbal
        from chartevents
        where itemid=223900
),
all_charts as
(select distinct *
	from (select hadm_id, subject_id, stay_id, charttime from gcs_eye
	union all
	select hadm_id, subject_id, stay_id, charttime from gcs_motor
	union all
	select hadm_id, subject_id, stay_id, charttime from gcs_verbal) as final)
...
all_charts.stay_id=gcs_eye.stay_id and
...
all_charts.stay_id=gcs_motor.stay_id and
...
all_charts.stay_id=gcs_verbal.stay_id and
