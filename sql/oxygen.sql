with HR as
(select hadm_id, subject_id,stay_id,  charttime, valuenum as pulse
        from chartevents
         where itemid in (220045, 220046, 220047)),
RR as
(select hadm_id, subject_id, stay_id, charttime, valuenum as respiratory_rate
        from chartevents
...
select
	...
	coalesce(HR.stay_id, RR.stay_id) as stay_id,
