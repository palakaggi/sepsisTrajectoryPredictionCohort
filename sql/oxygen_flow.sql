...
--with all_charts as(
with spo2Tab as
(select hadm_id, subject_id, charttime, valuenum as spo2
        from chartevents
         where itemid=220277),
fio2Tab as
(select hadm_id, subject_id, charttime, valuenum as fio2
        from chartevents
        where itemid=223835),
oxy_flow_rt as
(
select hadm_id, subject_id, charttime, valuenum as oxygen_flow_rate
	from chartevents
	where itemid in (223834,227287)
),
sao2Tab as
(
select hadm_id, subject_id, charttime, valuenum as sao2
	from chartevents
	where itemid=220227
),
all_charts as
(select distinct *
        from (select hadm_id, subject_id, charttime from spo2Tab
                union all
        select hadm_id, subject_id, charttime from fio2Tab
		union all
	select hadm_id, subject_id, charttime from oxy_flow_rt
		union all
	select hadm_id, subject_id, charttime from sao2Tab) as final)
