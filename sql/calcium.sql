...
with non_ion as
(
select subject_id,hadm_id, charttime, cast(value as numeric) as Cal_non_ionized_mg_per_dL
from chartevents 
where itemid=225625),
ionized as
(select subject_id,hadm_id,charttime, (cast(value as numeric)*4.01) as cal_ionized_mg_per_dL
from chartevents
where itemid=225667),
all_charts as
(select distinct *
	from (select hadm_id, subject_id, charttime from non_ion
		union all
	select hadm_id, subject_id, charttime from ionized) as final),
merge as(
