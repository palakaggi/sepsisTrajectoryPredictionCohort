...
with abea as
(
select subject_id,hadm_id, charttime, value as ABEA_mEq_per_L
from chartevents 
where itemid=224828),
anion_gap as
(select subject_id,hadm_id,charttime, value as anion_gap_mEq_per_L
from chartevents
where itemid=227073),
bicarb as
(
select subject_id,hadm_id, charttime, value as bicarb_mEq_per_L
from chartevents 
where itemid=227443),
bun as
(
select subject_id, hadm_id, charttime, value as bun_mg_per_dL
from chartevents 
where itemid in (225624, 227000)),
all_charts as
(select distinct *
        from (select hadm_id, subject_id, charttime from abea
                union all
        select hadm_id, subject_id, charttime from anion_gap
                union all
        select hadm_id, subject_id, charttime from bicarb
                union all
        select hadm_id, subject_id, charttime from bun) as final)
