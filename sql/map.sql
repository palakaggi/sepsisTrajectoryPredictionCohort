with line as
(select hadm_id, subject_id,stay_id, charttime, valuenum as map_line
        from chartevents 
         where itemid in (225312, 220052, 224322)),
cuff as
(select hadm_id, subject_id, stay_id,charttime, valuenum as map_cuff
        from chartevents 
        where itemid=220181),
first as (
select
	...
	coalesce(line.stay_id, cuff.stay_id) as stay_id,
