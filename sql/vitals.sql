with temp as 
(select hadm_id, subject_id, charttime, valuenum as temperature
	from chartevents 
	 where itemid in (223762,223761)),
sbp as 
(select hadm_id, subject_id, charttime, valuenum as sbp_line
	from chartevents 
	where itemid in (225309,220050)),
sbp2 as 
(select hadm_id, subject_id, charttime, valuenum as sbp_cuff
	from chartevents
	where itemid in (224167,227243,220179)),
dbp_line as
(select hadm_id, subject_id, charttime, valuenum as dbp_line
        from chartevents
        where itemid in (225310, 220051)),
dbp_cuff as
(select hadm_id, subject_id, charttime, valuenum as dbp_cuff
        from chartevents
